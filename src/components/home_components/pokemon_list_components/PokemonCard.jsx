...
const PokemonCard = ({ id, name, types, sprites, iconType }) => {
  return (
    <Link to={`/pokemon-detail/${id}`}>
      <div
        className="card text-white w-36 h-36 p-3 rounded-md hover:shadow-[0px_0px_15px_rgba(255,255,255,0.2)] transition duration-[50ms] ease-in-out border-2 border-dashed border-white/25 md:w-40 md:h-52"
        key={id}
      >
